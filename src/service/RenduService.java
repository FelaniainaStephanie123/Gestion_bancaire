...
public class RenduService {
    ...
    public boolean ajouterRemboursement(Rendu rendu) {
        ...
        if (rendu.getMontantPaye() == null || rendu.getMontantPaye().compareTo(BigDecimal.ZERO) <= 0) {
        ...
        if (rendu.getMontantPaye().compareTo(situation.getResteAPayer()) > 0) {
            System.out.println("Erreur : le remboursement dépasse la dette restante (reste à payer : " + situation.getResteAPayer() + ").");
            return false;
        }
    ...
    public boolean modifierRemboursement(Rendu rendu) {
        ...
        if (rendu.getNumRendu() == null || rendu.getNumRendu().isEmpty()) {
